...
import {
  ...
  CreateTaskClarificationPayloadType,
  ...
  ProjectResponseType,
  TaskClarificationListResponseType,
...
export const getTaskClarificationsQueryFn = async ({
  workspaceId,
  taskId,
}: {
  workspaceId: string;
  taskId: string;
}): Promise<TaskClarificationListResponseType> => {
  const response = await API.get(
    `/task/${taskId}/workspace/${workspaceId}/clarifications`
  );
  return response.data;
};

export const createTaskClarificationMutationFn = async ({
  workspaceId,
  taskId,
  question,
}: CreateTaskClarificationPayloadType): Promise<{
  message: string;
...
export const respondTaskClarificationMutationFn = async ({
  workspaceId,
  taskId,
  clarificationId,
  message,
}: RespondTaskClarificationPayloadType): Promise<{
  message: string;
  clarification: TaskClarificationResponseType;
}> => {
  const response = await API.post(
    `/task/${taskId}/workspace/${workspaceId}/clarifications/${clarificationId}/respond`,
    {
      message,
    }
  );
